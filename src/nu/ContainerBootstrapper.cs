...
namespace nu
{
	...
	public class ContainerBootstrapper
	{
		...
		public IContainer Bootstrap()
		{
			...
			IContainer container = new StructureMap.Container(x =>
				{
					...
					x.Scan(scan =>
						{
							...
							IEnumerable<Directory> nugextensions = from d in nugsDirectory.GetDirectories()
							                                       where d.GetChildDirectory("extensions").Exists()
							                                       select d.GetChildDirectory("extensions");

							nugextensions.Each(dd => { scan.AssembliesFromPath(dd.Name.GetPath()); });
